...
router.post("/carts/", async (req, res) => {
  ...
  try {
    ...
    req.logger.error(`Error adding Cart: ${e.message}`);
...
router.get("/carts/:cid", async (req, res) => {
  ...
  try {
    ...
    req.logger.info(`Return cart products: ${JSON.stringify(productsFromCart)}`);
    if (productsFromCart) {
    ...
    req.logger.error(`Error getting Cart by ID: ${e.message}`);
...
router.put("/carts/:cid", async (req, res) => {
  ...
  try {
    ...
    req.logger.error(`Error updating Cart by ID: ${e.message}`);
    res.status(400).send({ error: e, description: "Endpoint: GET cart/:cid" });
  }
});
...
router.post("/carts/:cid/product/:pid", async (req, res) => {
  ...
  try {
    ...
    req.logger.error(`Error adding Product by Cart: ${e.message}`);
    res
      .status(400)
...
router.put("/carts/:cid/product/:pid", async (req, res) => {
  ...
  try {
    ...
    req.logger.error(`Error updating product stock for a Cart: ${e.message}`);
    res
      .status(400)
      .send({ error: e, description: "Endpoint: POST /:cid/product/:pid" });
  }
...
router.delete("/carts/:cid/product/:pid", async (req, res) => {
  ...
  try {
    ...
    req.logger.error(`Error deleting Cart: ${e.message}`);
    res
      .status(400)
      .send({ error: e, description: "Endpoint: POST /:cid/product/:pid" });
  }
...
router.delete("/carts/:cid", async (req, res) => {
  ...
  try {
    ...
    req.logger.error(`Error cleaning Cart: ${e.message}`);
...
router.get("/carts/:cid/purchase", async (req, res) => {
  ...
  try {
    ...
    if (purchaseComplete.length > 0) {
      ...
      req.logger.info(`Purchase done with unprocessed products`);
    ...
    req.logger.error(`Purchase Error!: ${e.message}`);
